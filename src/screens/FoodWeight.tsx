...
import { BUTTON, CARD, FoodPush, LoadFailed, StatRow } from './FoodParts'
      ...
      <BootItem className={CARD}>
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          <input
            ...
            className="h-11 min-w-0 flex-1 rounded-ctl border border-line bg-surface px-3 text-center text-metric-sm font-mono tabular-nums text-ink placeholder:text-ink-faint focus:border-line-bright"
          />
          <span className="text-label text-ink-faint">kg</span>
          <button
            type="button"
            onClick={() => void log()}
            disabled={busy || !valid}
        ...
        <p className="mt-2 text-body text-ink-dim">
          ...
          the coach smooths it into a trend and reads that instead.
        </p>
      ...
      {latest && (
        <BootItem className={`mt-2.5 ${CARD}`}>
          ...
          <div className="mt-2">
            ...
            {coach.expenditure && (
              <StatRow label="Trend change" hint={`last ${coach.expenditure.windowDays} days`}>
            ...
            {recent.map((point) => (
              ...
              >
                <span className="text-body text-ink-dim">
                  {label(point.day)}
                  {point.day === today && <span className="ml-2 text-label text-live">today</span>}
                </span>
                <span className="flex items-center gap-3">
                  <span className="text-body font-mono tabular-nums text-ink">
                    {point.kg.toFixed(1)}
                    <span className="ml-2 text-label text-ink-faint">
                      trend {point.trendKg.toFixed(1)}
                    </span>
                  </span>
                  <button
                    type="button"
                    ...
                    disabled={busy}
                    aria-label={`Remove weigh-in for ${label(point.day)}`}
